...
function parse(text: string): { items: Item[]; invalidLines: number[] } {
  ...
  text.split(/\n|;/).forEach((source, index) => {
    ...
    if (match && (!Number.isFinite(amount) || amount! < 0)) { invalidLines.push(index + 1); return; }
...
function navigate(path: string) { history.pushState({}, '', path); demo = path === '/demo'; list = path === '/handoff' ? fresh() : load(); showQr = false; render(); window.scrollTo({ top: 0, behavior: 'instant' as ScrollBehavior }); requestAnimationFrame(() => document.querySelector<HTMLElement>('h1')?.focus()); }
window.addEventListener('popstate', () => { demo = location.pathname === '/demo' || new URLSearchParams(location.search).get('demo') === '1'; list = location.pathname === '/handoff' ? fresh() : load(); showQr = false; render(); });
...
function render() {
  ...
  const currentTitle = route === '/privacy' ? 'Privacy — Shopping List Handoff' : route === '/terms' ? 'Terms — Shopping List Handoff' : route === '/demo' ? 'Demo — Shopping List Handoff' : isHandoff ? 'Shared list — Shopping List Handoff' : 'Shopping List Handoff — Share a clear shopping list';
  document.title = currentTitle;
  const canonical = document.querySelector<HTMLLinkElement>('link[rel="canonical"]');
  if (canonical) canonical.href = `https://shopping-list-handoff.sociobot.in${route === '/' ? '/' : route}`;
  app.innerHTML = `${header()}${demo ? `<aside class="demo-banner" aria-label="Demo mode"><span><b>Demo</b> — sample data, nothing is saved.</span><button id="reset-demo">Reset demo</button><button id="start-real">Start for real</button></aside>` : ''}${isHandoff ? receivedHandoff() : isLegal ? legal(route.slice(1) as 'privacy' | 'terms') : landing()}${footer()}<div class="toast" role="status" aria-live="polite">${esc(toast)}</div>`;
